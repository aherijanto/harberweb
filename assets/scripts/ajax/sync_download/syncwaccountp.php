...
#by Ary Heirjanto
    ...
    foreach($json1 as $mydata){
        ...
        $sqlselect = "SELECT * FROM waccountp WHERE  id='$id';";    
        $resultselect = mysqli_query($con,$sqlselect);
        $num_row = mysqli_num_rows($resultselect);
    
        if($num_row == 0){
            $sqlinsert = "INSERT INTO waccountp (id,s_code,r_date,r_type,r_nocheque,r_amount) VALUES ('$id','$s_code','$r_date','$r_type','$r_nocheque','$r_amount')";
            $result = mysqli_query($con, $sqlinsert)or die("Couldn't execute query:<br>". mysqli_error($con));
       }else{
           $sqlupdate = "UPDATE waccountp SET 
            s_code = '$s_code',
            r_date='$r_date',
            r_type='$r_type',
